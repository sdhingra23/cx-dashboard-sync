...
export async function buildMetabaseData(questions, token) {
  ...
  for (const { cfg, rows } of results) {
    for (const row of rows) {
      ...
      // Map configured column names onto row values (case-insensitive, spaces→underscores)
      const rowKeyMap = Object.keys(row).reduce((m, k) => {
      ...
      for (const col of (cfg.columns || [])) {
        ...
        // columnMap lets you rename a Metabase column to an internal field name
        const targetKey = cfg.columnMap?.[col] ?? col;
        merged[acctName][targetKey] = row[rowKey];
      }
    }
  }
