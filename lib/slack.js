...
export async function postAccountFlagAlert(account, flagEntries, dashboardBase) {
  ...
  // Fixed alert emoji on the header — using a specific flag's emoji (e.g. a
  // calendar for renewal-at-risk) next to the company name reads as if it's
  // describing the company, not flagging an alert. Flag-specific emoji stay
  // on each flag's own line below, where the context is clear.
  const headerEmoji = '🚨';
