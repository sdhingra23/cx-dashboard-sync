...
export async function postAccountFlagAlert(account, flagEntries, dashboardBase) {
  ...
  const multi = flagEntries.length > 1;

  const headerEmoji = multi ? '🚨' : flagEmoji(flagEntries[0].flagKey);
  const title = multi ? `${flagEntries.length} flags triggered` : flagEntries[0].label;

  // Plain-text fallback (shown in notifications / clients that don't render blocks)
  const text = `${headerEmoji} ${title} — ${name}`;
  ...
  for (const { flagKey, label, metric } of flagEntries) {
    blocks.push({ type: 'divider' });
    blocks.push({
      type: 'section',
      text: { type: 'mrkdwn', text: `${flagEmoji(flagKey)} *${label}*\n*What triggered:*\n${metric}` },
    });
...
function flagAction(flagKey) {
  const map = {
    ...
    flag_login_stale_14:        'Check in with account contact — no logins in 2+ weeks',
    flag_login_stale_30:        'Proactive outreach; confirm the champion is still active',
    flag_login_stale_90:        'Escalate — account may be dormant, consider a risk review',
  };
