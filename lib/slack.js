...
export async function postAccountFlagAlert(account, flagEntries, dashboardBase) {
  ...
  // Fixed, neutral emoji on the header — using a specific flag's emoji (e.g.
  // a calendar for renewal-at-risk) next to the company name reads as if
  // it's describing the company, not flagging an alert; an alarm siren on
  // every message (including a single non-urgent flag) overstates it the
  // other way. 🏢 matches how the rest of the app marks "this is an
  // account" (account page title, brand dashboard header). Flag-specific
  // emoji stay on each flag's own line below, where urgency is contextual.
  const headerEmoji = '🏢';
