...
export function buildLocationConfigByAccount(rows) {
  ...
  for (const l of rows) {
    const a = acc[l.account_name] || (acc[l.account_name] = {
      ...
      loc_active_no_apps: 0, loc_boost_gap: 0, loc_jobs_no_salary: 0,
    });
    ...
    if (l.published_jobs > 0) {
      ...
      // "Starving": a job is posted and it produced nothing for a month.
      // Anchored at exactly zero rather than a low-water mark — applications
      // per job vary too much by role and market to justify a threshold, but
      // zero needs no calibration in any market or season.
      if ((l.total_apps_30d || 0) === 0) {
        a.loc_active_no_apps++;
        // A starving job that isn't being boosted is the boost gap: the one
        // case where NOT boosting is a genuine health problem. Locations with
        // healthy application flow can never land here, which is why the
        // score can reward boosting without punishing accounts that have no
        // need for it.
        if (!l.has_boosted_30d) a.loc_boost_gap++;
      }
    }
  ...
  for (const [name, a] of Object.entries(acc)) {
    out[name] = {
      ...
      perc_active_locs_no_apps: a.loc_active > 0
        ...
        : null,
      // Starving jobs that aren't boosted, as a share of locations that are
      // actually recruiting.
      perc_active_locs_boost_gap: a.loc_active > 0
        ? Math.round((a.loc_boost_gap / a.loc_active) * 1000) / 10
        : null,
      locs_boost_gap: a.loc_boost_gap,
    };
  }
  return out;
}
