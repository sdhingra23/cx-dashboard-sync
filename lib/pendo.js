...
// "*ALL Employers" segment — scopes the accounts source so account.auto.lastvisit
// (login staleness) is reliably populated across all accounts.
const ALL_EMPLOYERS_SEGMENT_ID = '4689849911410688';
 ...
 *
 * lastVisit is account.auto.lastvisit (epoch ms) — scoped to the "*ALL Employers"
 * segment (subId 4689849911410688), which is confirmed to keep it populated.
 */
export async function fetchAccountActivity(apiKey) {
  ...
  // that with "only a single source is allowed"). Segment scoping is a
  ...
  const body = {
    ...
    request: {
      pipeline: [
        ...
        {
          segment: { id: ALL_EMPLOYERS_SEGMENT_ID },
        },
        {
          select: {
            accountId:      'accountId',
