...
export async function fetchNpsResponses(apiKey) {
  ...
  for (const row of rows) {
    ...
    for (const poll of polls) {
      // Pendo NPS poll response contains `numericResponse` (0-10) and optional `freeTextResponse`
      const score = poll.numericResponse ?? poll.score;
      if (score === undefined || score === null) continue;

      responses.push({
        account_id:       String(row.accountId || ''),
        visitor_id:       String(row.visitorId  || ''),
        score:            Number(score),
...
export async function fetchAccountActivity(apiKey) {
  const [current, prior] = await Promise.all([
    fetchAccountActivityWindow(apiKey, 'now', -1),   // last 30 days
    fetchAccountActivityWindow(apiKey, 'now', -2),   // 30–60 days ago
  ]);

  // Index prior period by account ID
  const priorMap = {};
  for (const row of prior) {
    priorMap[row.accountId] = row;
  }

  const result = {};
  for (const row of current) {
    const id     = String(row.accountId || '');
    const priorRow = priorMap[id];
    result[id] = {
      ...
      pendo_days_active_per_visitor_prior:  priorRow?.daysActive ?? null,
...
async function fetchAccountActivityWindow(apiKey, anchor, count) {
  const body = {
    response: { mimeType: 'application/json' },
    request: {
      pipeline: [
  ...
  return pendoAggregate(apiKey, body);
}
