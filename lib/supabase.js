...
//
// Always orders by account_name so range-based paging is stable — without
// an explicit order, Postgres doesn't guarantee row order is the same across
// two separate range() calls (rows shift after concurrent updates), which
// can silently skip or duplicate rows across pages. Every table this helper
// is called against (accounts, snapshots, nps_responses) carries account_name.
async function paginateAll(table, select, extraQuery, orderCol = 'account_name') {
  const PAGE = 500;
  const sb = client();
  let all = [];
  let page = 0;
...
/**
 * Fetch each account's most recent snapshot strictly before today.
 * Returns a map: { account_name: snapshotRow }
 *
 * Deliberately NOT an exact "today - 1 day" match — the daily sync
 * (GitHub Actions `schedule`) can silently skip or fail on a given day,
 * which would otherwise make this return nothing for that account even
 * though a snapshot exists from 2+ days ago. Falling back to the latest
 * snapshot in a lookback window keeps the newlyTriggered diff in
 * computeFlags() correct across gaps — long-lived flags (e.g. login
 * staleness) would otherwise look "newly triggered" again after any
 * missed run and re-fire the same Slack alert.
 */
export async function getYesterdaySnapshots() {
  ...
  const rows = await paginateAll(
    'snapshots',
    '*',
    q => q.gte('snapshot_date', fromStr).lt('snapshot_date', today).order('snapshot_date', { ascending: false })
  );

  const map = {};
  for (const row of rows) {
    // rows are newest-first, so the first one seen per account is the most recent.
    if (!map[row.account_name]) map[row.account_name] = row;
  }
  return map;
}

/**
 * Fetch snapshots for the date N days ago as an account_name → row map.
 * Returns an empty map if no snapshots exist for that date (e.g. first run).
 */
export async function getSnapshotNDaysAgo(n) {
  const d = new Date();
  d.setUTCDate(d.getUTCDate() - n);
  const dateStr = d.toISOString().split('T')[0];

  const rows = await paginateAll('snapshots', '*', q => q.eq('snapshot_date', dateStr));
  const map = {};
  for (const row of rows) map[row.account_name] = row;
  return map;
}
...
export async function getSnapshotRange(fromDate, toDate) {
  return paginateAll('snapshots', '*', q => q.gte('snapshot_date', fromDate).lte('snapshot_date', toDate));
}
...
export async function upsertNpsResponses(responses) {
  ...
  const rows = responses.map(r => ({
    ...
    role_raw:         r.role_raw  || null,
    role_tier:        r.role_tier || null,
  }));
  const { error } = await sb
    .from('nps_responses')
...
export async function getNpsResponseRange(fromDate, toDate) {
  return paginateAll('nps_responses', '*', q => q.gte('response_date', fromDate).lte('response_date', toDate));
}
...
/**
 * Upsert location drill-down rows (one row per location, keyed on location_id).
 * Written in batches — a full sync can carry tens of thousands of locations,
 * which exceeds what PostgREST will accept in a single request.
 */
export async function upsertLocations(locations) {
  ...
  for (let i = 0; i < locations.length; i += BATCH) {
    ...
    const { error } = await sb
      .from('locations')
      .upsert(chunk, { onConflict: 'location_id' });
    if (error) throw new Error(`upsertLocations (batch ${i / BATCH + 1}): ${error.message}`);
  }
  console.log(`Supabase: upserted ${locations.length} locations`);
}

/**
 * Delete location rows that this run didn't touch — locations that have been
 * closed or moved to another account since the last sync.
 *
 * Guarded on writtenCount the same way deleteStaleAccounts() is: if the
 * Metabase question failed or returned a partial result, an unguarded prune
 * would wipe the table.
 */
export async function deleteStaleLocations(syncedAt, writtenCount) {
  if (writtenCount < 10) {
    console.warn(`deleteStaleLocations: only ${writtenCount} locations written — skipping prune as a safety guard.`);
    return;
  }
  const sb = client();
  const { data, error } = await sb
    .from('locations')
    .delete()
    .lt('last_synced', syncedAt)
    .select('location_id');
  if (error) throw new Error(`deleteStaleLocations: ${error.message}`);
  console.log(`deleteStaleLocations: removed ${data?.length || 0} stale locations.`);
}
