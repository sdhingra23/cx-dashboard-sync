 ...
 * @returns {Array<{account_name, account_id, email, arr, outstanding_balance, cb_customer_count, create_date, renewal_date, account_manager, parent_brand}>}
 */
export async function buildChargebeeData(apiKey) {
  ...
  for (const c of customers) {
    ...
    // AM assignment — source of truth is the cf_account_manager custom field
    // (blank for accounts not yet tagged in Chargebee; sync.js falls back to
    // the CSV for those).
    const cfAccountManager = (c.cf_account_manager || '').trim() || null;
    // Parent brand — groups multiple franchisee/location accounts under one
    // brand rollup (e.g. "Dunkin'"). Blank for independent accounts.
    const cfParentBrand = (c.cf_parent_brand || '').trim() || null;
    ...
    if (!rollup[name]) {
      ...
      rollup[name] = {
        ...
        account_manager:     cfAccountManager,
        parent_brand:        cfParentBrand,
      };
    } else {
      ...
      // First customer (in fetch order) with a non-empty cf_account_manager wins —
      // a multi-customer account should have the same AM tagged on every
      // customer record, but this avoids a later blank customer clobbering it.
      if (!rollup[name].account_manager && cfAccountManager) {
        rollup[name].account_manager = cfAccountManager;
      }
      if (!rollup[name].parent_brand && cfParentBrand) {
        rollup[name].parent_brand = cfParentBrand;
      }
    }
