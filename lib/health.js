...
export function computeHealthScore(acc) {
  // ── 1. Applicant pipeline health (20%) ────────────────────
  ...
  if (!acc.is_zero_roi) {
    ...
    pipelineScore     = (healthPct / 100) * 20;
  }

  // ── 2. Platform activity (20%) ────────────────────────────
  // 4 connected signals — each worth 5 pts.
  ...
  const activityScore  = (activeCount / activitySignals.length) * 20;
  ...
  const billingScore = (Number(acc.billing_balance_effective ?? acc.outstanding_balance) || 0) > 0 ? 0 : 12;

  // ── 5. Pendo engagement (12%) ─────────────────────────────
  // Based on days_active_per_visitor. No Pendo data → neutral (6 / 12).
  ...
  if (!isNaN(daysActive) && daysActive > 0) {
    engagementScore = Math.min(daysActive / 20, 1) * 12;
  }

  // ── 6. CX gut score (20%) ─────────────────────────────────
  // Manual 0–10 AM assessment (accounts.cx_gut_score). No score yet → neutral (10/20).
  let gutScore = 10;
  if (acc.cx_gut_score !== null && acc.cx_gut_score !== undefined) {
    const rawGut = Number(acc.cx_gut_score);
    if (!isNaN(rawGut)) gutScore = (Math.min(10, Math.max(0, rawGut)) / 10) * 20;
  }
