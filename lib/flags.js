...
export function computeFlags(account, yesterday, recentVerbatims = []) {
  ...
  // ── 5. Paid feature lapsed ──────────────────────────────────
  const jbLapsed =
    Boolean(account.job_boost_enabled) &&
    (Number(account.job_boost_last_used_days) || 0) >= 60;
  const nmLapsed =
    Boolean(account.feature_nextmatch) &&
    (Number(account.nextmatch_calls_90d) || 0) === 0;
  flags.flag_paid_feature_lapsed = jbLapsed || nmLapsed;
...
export const FLAG_LABELS = {
  ...
  flag_paid_feature_lapsed:   'Paid feature lapsed (60+ days unused)',
