...
export function computeFlags(account, yesterday, recentVerbatims = []) {
  ...
  // ── Scoring-model cutover guard ──────────────────────────────
  // A change to the health formula moves every account's score at once. Both
  // drop flags are URGENT and post to Slack immediately, so without this the
  // first run on a new model alerts on most of the book in one morning for
  // accounts whose actual health has not changed. Snapshots carry the model
  // version that produced them; when it differs from today's, the two scores
  // are not comparable and the drop flags are skipped for that run only.
  //
  // A NULL previous version counts as changed, not as "can't tell". Snapshots
  // written before versioning existed have no version at all, so requiring
  // both sides to be non-null made the guard fail open on exactly the
  // transition it was built for — the v1→v2 cutover fired 893 flags and 130
  // Slack alerts in one run because of it.
  const modelChanged =
    yesterday !== null &&
    account.score_model_version != null &&
    Number(yesterday.score_model_version ?? -1) !== Number(account.score_model_version);
  ...
  // Based on account.auto.lastvisit (pendo_last_active), fetched via lib/pendo.js.
  // No lastvisit data → never flagged. Bands are mutually exclusive (not
  // cumulative >= thresholds) so a 200-day-dormant account only trips
  // flag_login_stale_90 — not all three at once, which would triple-alert
  // Slack the day it's first crossed.
  const daysSinceLogin = account.pendo_last_active
    ? Math.floor((Date.now() - new Date(account.pendo_last_active).getTime()) / 86400000)
    : null;
  flags.flag_login_stale_14 = daysSinceLogin !== null && daysSinceLogin >= 14 && daysSinceLogin < 30;
  flags.flag_login_stale_30 = daysSinceLogin !== null && daysSinceLogin >= 30 && daysSinceLogin < 90;
  flags.flag_login_stale_90 = daysSinceLogin !== null && daysSinceLogin >= 90;

  // ── Diff: which flags are newly true vs yesterday ─────────────
  // Every flag except churn_verbatim is a day-over-day comparison, not a
  // fresh-today event — "newly triggered" only means something relative to
  // a KNOWN prior state. An account with no prior snapshot at all (its first
  // day ever being tracked — e.g. newly onboarded, or newly entering the
  // Chargebee-driven universe after being invisible to the dashboard before)
  // would otherwise have every pre-existing condition it happens to be in
  // reported as "newly triggered," dumping its whole history into Slack in
  // one run as if it all just happened today. churn_verbatim is exempt: it
  // reflects an NPS response in the last 24h, which is a real same-day event
  // regardless of how long the account has been tracked.
  const newlyTriggered = {};
  for (const [key, val] of Object.entries(flags)) {
    const wasTrue = Boolean(yesterday?.[key]);
    const isFreshSignal = key === 'flag_churn_verbatim';
    newlyTriggered[key] = Boolean(val) && !wasTrue && (isFreshSignal || yesterday !== null);
  }
